describe('Before version 0.5', () => {
  beforeAll(async () => {
    const _ = await (async () => {
      globalThis.mockApi = SimplifiedFetch.default.create({
  ...
  describe('Dynamic config', () => {
    test('Dynamic config', async () => {
      const _ = await (async () => {
        ...
        try {
          await Api.urnStringTest(null, null, {
            method: 'POST',
            custom: {
              customTag: true
            }
    ...
    test('Config custom with object.assign', async () => {
      const _ = await (async () => {
        ...
        try {
          await Api.customConfigTest(null, null, {
            custom: {
              customTag: false,
              tag: true
            }
    ...
    test('In PipeRequest', async () => {
      const _ = await (async () => {
        ...
        try {
          await Api.urnStringTest(undefined, undefined, {
            method: 'POST'
          });
        } catch (e) {}
  ...
  describe('Auto transforming of params & body', () => {
    describe('params', () => {
      test('append pathname: Array, String, Number', async () => {
        ...
        expect(_).toEqual([['/test/0,1', 'none'], ['/test//01', 'none'], ['/test/0', 'none']]);
      });
      test('append search: Object, FormData, URLSearchParams', async () => {
        const _ = await (async () => {
          ...
          try {
            await Api.urnStringTest(undefined, {
              a: 0,
              b: 1
            });
          } catch (e) {}
          ...
          try {
            await Api.urnStringTest(undefined, new URLSearchParams({
              a: 0,
              b: 1
            }));
          } catch (e) {}
    ...
    describe('body', () => {
      ...
      test('append search: Object, FormData, URLSearchParams', async () => {
        const _ = await (async () => {
          ...
          try {
            await Api.urnStringTest(new URLSearchParams({
              a: 0,
              b: 1
            }));
          } catch (e) {}
    ...
    describe('params and body transforming order', () => {
      test('params pathname & body pathname', async () => {
        ...
        expect(_).toEqual([['/test/0,1/2,3', 'none']]);
      });
      test('params pathname & body search', async () => {
        ...
        expect(_).toEqual([['/test/0,1', '?a=0&b=1', 'none']]);
      });
      ...
      test('params search & body search', async () => {
        ...
        expect(_).toEqual([['?a=0&b=1&c=2&d=3', 'none']]);
      });
    });
  });
  describe('Pipe with order', () => {
    describe('PipeRequest', () => {
      test('use & eject', async () => {
        const _ = await (async () => {
          const final = [];
          const NoRequest = Api.request.use(0b111, (...args) => {
            final.push(0b111);
            return pipes.NoRequest(...args);
          });
          const order1 = Api.request.use(0b1, () => {
            final.push(1);
          });
          const order0 = Api.request.use(() => {
            final.push(0);
          });
    ...
    describe('PipeResponse', () => {
      test('use & eject', async () => {
        const _ = await (async () => {
          ...
          const order00 = mockApi.response.use(0, () => {
            final.push('00');
          });
          const order0 = mockApi.response.use(() => {
            final.push(0);
          });
    ...
    test('Should each PipeResponse gets unique Response and Resquest?', async () => {
      ...
      const _ = await (async user => {
        const final = [];
        const use = mockApi.response.use((res, req) => {
          res.json();
          req.json();
        });
